...
function loadModel(){
	chrome.storage.sync.get('trained', function(result) {
		var message = (result.trained?"The model has been trained":"The mode has NOT been trained")
		console.log(message);
		if (result.trained){
...
window.pointer = {
    ...
    movePointer: function(){
		...
		// //will likely need a try statement
		// if (model_trained == false){
		// 	current_model = await tf.loadLayersModel('localstorage://best-model');	
		// }
        ...
        if (current_model == null || model.isTraining){
          return;
        }
...
window.model ={
	...
	createCNNModel: function() {
		...
		//flatten the data for faster processing
		...
		await model.current_model.fit(data.training_data.a, data.training_data.b, {
			...
			callbacks: {
				...
				onTrainEnd: async function() {
					...
					current_model = await tf.loadLayersModel(bestModelPath);
				},
			},
		});
	},
